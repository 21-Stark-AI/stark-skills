...
| `resume-target` | `[--slug]` + injected PR-state reader | Prints `{run_id, slug, target_stage, action: reinvoke \| advance \| complete \| merge_only, reconciled?: boolean}` — the next stage to run after reconciliation. `merge_only` = the stage's execution already completed and only the pending artifact merge remains (the `fold_pr_open` halt and the crash-mid-merge `merge_pending` case), so resume retries the merge without re-invoking the stage skill. |
...
3. For each stage in order:
   ...
   f. Advance to the next stage. A stage that begins a **new artifact** (spec-to-plan, plan-to-tasks, copilot) runs against `main` updated by the prior artifact's merge — **forge performs the base sync itself** before transitioning that stage to `running`: `git switch <default-branch>` + `git pull --ff-only`; a failed sync fails the incoming stage (`gate.reason = "base_sync_failed"`) rather than running it against a stale base. A stage that continues the **same artifact** (review-spec, red-team-spec, review-plan, red-team-plan) runs against the still-open shared PR branch — no sync.
...
- Resume acts on `resume-target`'s **`action`**, never unconditionally:
  ...
  - **`merge_only`** — the stage's execution already completed (its `record-output` checkpoint exists) and only the artifact merge remains (`fold_pr_open` halt, or `merge_pending` after a crash mid-merge). Resume transitions `halted → running`, re-checks the registry PRs / fold-PR live state, and **retries only the merge** — the stage skill is **not** re-invoked.
