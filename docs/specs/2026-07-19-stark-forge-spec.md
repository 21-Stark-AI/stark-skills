...
| `resume-target` | `[--slug]` + injected PR-state reader | Prints `{run_id, slug, target_stage, action: reinvoke \| advance \| complete \| merge_only, reconciled?: boolean}` — the next stage to run after reconciliation. `merge_only` is **derived from state**: a merge-point stage with a `record-output` completion checkpoint and an incomplete merge (regardless of gate reason — `fold_pr_open`, `merge_pending`, or `ci_red`) resumes by retrying only the merge, never re-invoking the stage skill; `reinvoke` = a halted/failed stage without a completion checkpoint. |
...
3. For each stage in order:
   ...
   f. Advance to the next stage. A stage that begins a **new artifact** (spec-to-plan, plan-to-tasks, copilot) runs against `main` updated by the prior artifact's merge — **forge performs the base sync itself, as the stage's first act after its `pending → running` transition** (so the failure path uses the legal `running → failed` move): `git switch <default-branch>` + `git pull --ff-only`; a failed sync → `running → failed` (`gate.reason = "base_sync_failed"`) before the stage skill is invoked, rather than running it against a stale base. A stage that continues the **same artifact** (review-spec, red-team-spec, review-plan, red-team-plan) runs against the still-open shared PR branch — no sync.
