...
**Triage orchestrator** — Central script (`scripts/triage_orchestrator.py`) that owns the triage → dispatch → TUI → telemetry flow. Single entry point called by the review skills (stark-review, stark-review-spec, stark-review-plan) with || fallback to direct dispatch.
