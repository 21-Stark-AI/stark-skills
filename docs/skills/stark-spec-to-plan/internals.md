...
![Internal architecture diagram for the stark-spec-to-plan skill showing a five-phase pipeline from CLI input and setup validation through three-agent plan generation, cross-review scoring, local synthesis, and final output persistence, with callouts for quorum decisions, degraded failure paths, observability requirements, configuration flags, and extension points."}}](internals.png)
...
Edit `skill/stark-spec-to-plan/SKILL.md`, then run `/stark-generate-docs --skill stark-spec-to-plan` to regenerate documentation.
