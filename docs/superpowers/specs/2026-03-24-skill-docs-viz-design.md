...
```json
{
  ...
  "skills": {
    "stark-review": {"hash": "def456", "quality": "ok"},
    "stark-session": {"hash": "789abc", "quality": "needs-human-review"}
  }
}
```
...
Skills flagged `needs-human-review` are always treated as stale and regenerated.

### Markdown Doc Generation

After viz selection, the script generates markdown docs. Each doc includes both **Mermaid diagrams** (searchable, accessible, render natively on GitHub) and **PNG embeds** (rich visual detail).

**usage.md template:**
- Title, one-line description
- "When to Use" — trigger phrases from frontmatter description
- Mermaid workflow overview diagram (generated by the LLM alongside the HTML)
- Rich visualization PNG embed
- Prerequisites (tools, auth, config needed)
- Arguments table
- Quick start example invocation
- Common patterns (real-world usage examples)
- Troubleshooting (top failure modes and what to do)
- Related skills

**internals.md template:**
- Title, architecture summary
- Mermaid phase flow diagram
- Rich visualization PNG embed
...
- How to modify this skill (not "extension points" — these are markdown files, not plugin APIs)
...
Before evaluation/screenshot, validate each generated HTML:
1. Must contain `<html>` and `</html>`
2. Must not contain `<script>` tags (stripped automatically)
3. Must not reference external URLs (fonts, CDNs)
4. Must contain at least one CSS class from the design system
...
| LLM returns invalid HTML | Strip `<script>` tags, re-validate. If still invalid, disqualified |
