...
**Hard rule:** If you are about to write a finding and you have already produced a finding about the same section or the same root cause in a previous domain, STOP. Do not write it. The previous domain's finding covers it. This is the single most common source of noise in design reviews — the same issue (e.g., "shell sandbox is insufficient") appearing in security, general, completeness, resilience, and scope. One finding is signal. Five findings about the same thing is noise.
