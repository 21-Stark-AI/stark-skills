...
- **Process analytics + circuit breakers:** every run computes per-round
  ...
  **refractions of one root cause dedup into a single finding**
  ...
  + dispositions** (fixed / skipped-as-trade-off / deferred) thread into later
  rounds so resolved concerns are not re-derived; the wing fixes
  **deletion-first**, and a fix round growing the doc past
  `compress_retry_growth_ratio` (default 1.15x) gets an in-round compress pass
  before commit. When the spec declares its bars (acceptance criteria /
