...
1. Dispatch the multi-agent review:
   ```bash
...
If CI fails: merge anyway with `--admin`, flag `ci_bypassed: true` in the run log.
...
Log recommendations to the run-log file. Suggest `/stark-review-improvement` if any threshold exceeded.

---

## Logging

Standard logging: create task, emit timestamped progress logs, record metrics block (phase, repo, task counts, duration, findings per task, agent scores, ci_bypassed, improvement flags). See [references/observability.md](references/observability.md) for the full protocol.
