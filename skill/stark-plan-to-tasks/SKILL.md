...
**No longer created as labels** (now tracked as project fields): `sp:*`, `risk:*`, `confidence:*`.
...
gh api /repos/{ORG}/{REPO}/issues \
  ...
  --field labels='["type:feature","stark-plan-to-tasks","plan:{PLAN_SLUG}"]' \
  --field type="{GH_ISSUE_TYPE}"
```

#### Project Integration (after issue creation)

After creating each task issue and receiving the issue number:

1. **Load project config:** Read `.github/project-config.json` from the target repo for `project_id` and field IDs. If file doesn't exist, warn "No project config found — skipping project integration" and continue without project fields.
...
2. **Get issue node ID:**
   ...
   ISSUE_NODE_ID=$(gh api graphql -f query='query($owner:String!,$repo:String!,$number:Int!){repository(owner:$owner,name:$repo){issue(number:$number){id}}}' \
     -f owner="$ORG" -f repo="$REPO" -F number=$ISSUE_NUMBER --jq '.data.repository.issue.id')
   ```
...
4. **Set fields** (one mutation per field, 100ms delay between):
   - Status → `backlog` (or `needs spec` if acceptance criteria section is empty)
   - Phase → plan phase name
   - Story Points → from task decomposition
   - Risk → from task decomposition
   - AI Suitability → classified by LLM during decomposition (autonomous/assisted/human-led based on risk, ambiguity, spec completeness)
   - Documentation State → `not started`
   - Spec Approval → `not required` (low/medium risk) or `pending` (high risk)
   - Release Approval → `not required`
   - Priority → from task decomposition or default `medium`

5. **Label changes:** Continue creating `type:*`, `plan:{SLUG}`, `stark-plan-to-tasks` labels. STOP creating `sp:*`, `risk:*`, `confidence:*` labels — these are now project fields.

6. **Error handling:** If any GraphQL call fails, log warning and continue. The issue (REST) was already created successfully — project integration is additive.
...
## Constraints
{technical, timeline, or policy constraints}

## Rollout
{deployment approach — required for medium/high risk}

## Rollback
{how to undo — required for medium/high risk}

## Required Artifacts
{documents, tests, configs that must be produced}

## Verification
{verification plan — required for medium/high risk}

## Artifacts
{links to PRs, CI results, docs — populated during execution, initially empty}
...
- Create GitHub Projects or milestones (but does add issues to an existing project if configured)
