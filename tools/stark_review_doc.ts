...
async function run(cmd: string, args: string[], opts: RunOptions): Promise<RunResult> {
  ...
  const result = await new Promise<RunResult>(
    (resolve) => {
      ...
      const timer = setTimeout(() => {
        // Don't synthesize the result + resolve on the timer. Kill the
        // child and wait for child.on("close") to populate `closed` —
        // resolving on stdio-end alone would let the result settle before
