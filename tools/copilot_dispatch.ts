...
export async function run(
  ...
  const result = await new Promise<RunResult>((resolve) => {
    ...
    const timer = setTimeout(() => {
      // Kill the child but DON'T resolve here. The staged result waits for
      // child.on("close") (and stdout/stderr "end") via tryFinish()'s
      // processClosed gate. Resolving on stdio-end alone is unsafe — a child
      // can close its FDs while the process keeps running.
      timedOutFlag = true;
    ...
    // Await stdout/stderr "end" before resolving so the final bytes are
    // captured before the result is returned.
