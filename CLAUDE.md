...
- `scripts/github_app.py` — multi-app GitHub auth (stark-claude, stark-codex, stark-gemini). Source-of-truth for **Python orchestrators only** (`preflight.py`, `runtime_env.py`, `github_projects.py`, `multi_review.py`, `autopilot_dispatch.py`, `plan_to_tasks_validate.py`). All SKILL.md shell invocations were swept to `tools/github_app.ts` on 2026-05-18 — see TS tools section.
