...
- `/stark-terraform-review [path] [--changed] [--fix] [--pr N] [--no-tools] [--min-severity ...]` — ad-hoc Terraform/OpenTofu (HCL) code review against a path (not a PR): runs the mechanical gates (`terraform fmt/validate`, `tflint`, `trivy config`, `checkov`) then a failure-mode manual pass (identity churn, secret exposure, blast radius, state safety, module contracts, testing gaps). **Pure-guidance skill — no TS dispatcher**, no tool deps; checklists in `skill/stark-terraform-review/references/` (`review-checklist.md`, `llm-mistakes.md` incl. a version→feature guard, `tooling.md`). Review-only unless `--fix`. Rules adapted from HashiCorp Style Guide (MPL-2.0) + Anton Babenko terraform-skill (Apache-2.0) + TerraShark; research in `docs/specs/2026-06-24-terraform-terragrunt-review-research.md`.
- `/stark-terragrunt-review [path] [--changed] [--fix] [--pr N] [--no-tools] [--min-severity ...]` — ad-hoc Terragrunt **orchestration-layer** review (include/dependency/generate/remote_state blocks, mock outputs, DAG cycles, values/DRY pattern, state isolation, stack composition, git source refspec). Maps the DAG via `terragrunt find --dag --dependencies`; defers resource/module HCL to `/stark-terraform-review` (boundary in `references/terraform-vs-terragrunt.md`). **Pure-guidance skill — no TS dispatcher.** Review-only unless `--fix`. Adapted from jfr992/terragrunt-skill (Apache-2.0) + TerraShark.
