...
- `tools/red_team_backfill_lib.ts` + `tools/red_team_backfill.ts` — TS port of `scripts/red_team_backfill.py` (Phase 5a slice 3). Pulls historical `red_team_runs` + `red_team_findings` rows out of the audit DB, builds matching insights envelopes (`red_team_run` / `red_team_finding` / `red_team_fix_plan`), and enqueues via the same `red_team_emit_queue_cli.py` seam the dispatcher uses. `--scope all|legacy|forward`, `--limit`, `--dry-run`, `--manifest`. Parity: `red_team_backfill_parity.test.ts` (includes a cross-language interop test that proves TS-enqueued envelopes collide on dedupe_key with Python's).
